...
class AliAlgPoint : public TObject
{
 public:
  ...
  void       SetELossVaried(Bool_t v=kTRUE)           {SetBit(kVaryELossBit);}
  void       SetConstainsMaterial(Bool_t v=kTRUE)     {SetBit(kMaterialBit);}
  void       SetConstainsMeasurement(Bool_t v=kTRUE)  {SetBit(kMeasurementBit);}
  void       SetUseBzOnly(Bool_t v=kTRUE)             {SetBit(kUseBzOnly);}
  //
  void       GetResidualsDiag(const *double pos, double *resid) const;
  //
  void       SetAlpha(double a)            {fAlpha = a;}
  void       SetXYZTracking(double r[3])   {for (int i=3;i--;) fXYZTracking[i] = r[i];}
  //
  Double_t*  GetDerivWS()            const {return (Double_t*)fDerivWS;}
 ...
 protected:
  ...
  //
  Double_t fParamWS[5];                                //! workspace for tracks params at this point
  Double_t fDerivWS[kNDerivWS];                        //! workspace for derivatives calculation
  //
